...
def create_department_lab(db: Session, department_lab: schema.DepartmentLabCreate):
...
def update_department_lab(db: Session, department_lab_vcode: str, department_lab: schema.DepartmentLabUpdate):
    ...
    db_department_lab.vmodified_by = department_lab.vmodified_by
    db_department_lab.dsort_at = datetime.utcnow()
...
def delete_department_lab(db: Session, department_lab_vcode: str):
    db_department_lab = get_department_lab_by_code(db, vcode=department_lab_vcode)
    if db_department_lab:
        db_department_lab.nstatus = 0
        db_department_lab.vmodified_by = "system"
        db_department_lab.dsort_at = datetime.utcnow()
        db.commit()
        db.refresh(db_department_lab)
    return db_department_lab
